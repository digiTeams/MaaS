...
class Column_Row:
    ...
    '''限制主问题，含有松弛变量'''
    def RMP_relaxed(self, theta, pre_dict, xindex, obj_choice, omega, matchings):
        s_t = time.time()
        # 建立模型
        ...
        ### 不同目标函数
        if obj_choice == 'double':
            print('RSS算法目标函数：采用加权目标函数，Omega取值为{}'.format(omega))
            ...
            print('RSS算法目标函数：仅最小化阻塞对数量')
            # 只考虑最小化阻塞对的数量，即M为-inf
            obj = 0
            for key in xindex.keys():
                obj += alpha[key]
            model.setObjective(obj, GRB.MINIMIZE)
        elif obj_choice == 'zero':
            print('RSS算法目标函数：仅最大化社会福利')
            # 只考虑最大化成本节约值，即M=0
            obj = x.prod(xindex)
            model.setObjective(obj, GRB.MAXIMIZE)
        else:
            print('RSS算法目标函数：仅最大化平台收入')
            # 加权目标函数，但是只考虑最最大化平台收益
            obj = 0
            for (s, j, i1, i2, i3) in xindex.keys():
                rev = (matchings[s][s, j, i1, i2, i3]['wel'] - matchings[s][s, j, i1, i2, i3]['save_d'] -
                       sum(matchings[s][s, j, i1, i2, i3]['save_r']))
                obj += (rev * x[s, j, i1, i2, i3] + omega * alpha[s, j, i1, i2, i3])
            model.setObjective(obj, GRB.MAXIMIZE)
    ...
    '''限制主问题'''
    def RMP(self, theta, pre_dict, xindex, obj_choice, matchings):
        s_t = time.time()
        # 建立模型
        model = Model('RMP problem')
        # 设置不显示求解的log日志信息
        ...
        x = model.addVars(xindex.keys(), vtype=GRB.BINARY, name='x')  # 匹配对应的自变量
        ############ 目标函数值
        if obj_choice == "platform":
            print('BFS算法目标函数：仅最大化平台收入')
            obj = 0
            for (s, j, i1, i2, i3) in xindex.keys():
                rev = (matchings[s][s, j, i1, i2, i3]['wel'] - matchings[s][s, j, i1, i2, i3]['save_d'] -
                       sum(matchings[s][s, j, i1, i2, i3]['save_r']))
                obj += rev * x[s, j, i1, i2, i3]
            model.setObjective(obj, GRB.MAXIMIZE)
        else:
            print('BFS算法目标函数：仅最大化社会福利')
    ...
    def exact(self, method, obj_choice, matchings):
        s_t = time.time()
        flag = True
        ...
        while flag:
            ...
            # 求解RMP问题
            rmp_flag, obj, sol, pi_r, pi_d = self.RMP(theta, pre_dict, xindex, obj_choice, matchings)
            # 如果RMP问题无可行解
            if rmp_flag == False:
            ...
            if len(sub_sol) != 0:
                ...
                flag = False
            e_t = time.time()
            if e_t - s_t >= 3600 * 10:      # 如果运行时间超过4个小时则直接停止运行
                obj, sol = 0, []
                break
        welfare = 0  # 计算最终匹配结果对应的VMT savings
        for key in sol:
            welfare += xindex[key]
        print('exact stable matching solution: obj {}, number of matches {}'.format(
                welfare, len(sol)))
        return welfare, sol, iter, len(theta)

    '''========the relaxed blocking searching algorithm 主框架========='''
    def approximate(self, method, obj_choice, omega, matchings):
        s_start = time.time()
        flag = True
        ...
        while flag:
            ...
            # print('--------------第{}次迭代--------------'.format(iter))
            # 求解RMP问题
            obj, sol, pi_r, pi_d, num_bro, mat_bro = self.RMP_relaxed(theta, pre_dict, xindex, obj_choice, omega, matchings)
            ...
            s_end = time.time()
            # 判断时间是否达到设置的停止时间
            if s_end - s_start < 36000:     # 如果没达到停止时间
        ...
        for key in sol:
            obj += xindex[key]
        print('approximate stable matching solution: obj {}, number of matches {}, number of blocking matches {}'.format(
            obj, len(sol), num_bro
        ))
