...
def optimal_stable(matchings, mat_list, r_ins, d_ins, pre_r, pre_d):
    ...
    # 模型求解及解的输出处理
    s_2 = time.time()
    # model.setParam('TimeLimit', 3600*6)     # 设置运行时间上限，单位为秒，总共6个小时
    model.optimize()
    e_2 = time.time()
...
def centralized_matching(matchings, mat_list, r_ins, d_ins):
    ...
    print('Centralized optimal matching solution: obj {}, number of matches {}'.format(cen_obj, len(cen_result)))
    return round(cen_obj, 2), cen_result



def dynamic_centralized(matchings, mat_list, r_ins, d_ins, remain_rider, remain_driver, pre_r, pre_d):
    # 建立自变量索引
    xindex = {}
    for (s, j, i1, i2, i3) in mat_list:
        xindex[s, j, i1, i2, i3] = matchings[s][s, j, i1, i2, i3]['wel']
    # 建立模型
    model = Model(' dynamic optimal centralized matching problem')
    # 设置不显示求解的log日志信息
    model.setParam('OutputFlag', 0)
    x = model.addVars(xindex.keys(), vtype=GRB.BINARY, name='x')  # 自变量
    model.setObjective(x.prod(xindex), GRB.MAXIMIZE)  # 目标函数
    ###### 约束条件
    # 对于每个剩余的司机而言，只能选择一个匹配，否则不能匹配
    for j in d_ins:
        if j in remain_driver:
            model.addConstr(x.sum('*', j, '*', '*', '*') <= 1, name='driver_{}'.format(j))
        else:
            model.addConstr(x.sum('*', j, '*', '*', '*') == 0, name='driver_{}'.format(j))
    # 对于每个剩余乘客而言，只能选择一个匹配；否则不能匹配
    for i in r_ins:
        if i in remain_rider:
            model.addConstr(
                x.sum('*', '*', i, '*', '*') + x.sum('*', '*', '*', i, '*') + x.sum('*', '*', '*', '*', i) <= 1,
                name='rider_{}'.format(i)
            )
            ...
            model.addConstr(
                x.sum('*', '*', i, '*', '*') + x.sum('*', '*', '*', i, '*') + x.sum('*', '*', '*', '*', i) == 0,
                name='rider_{}'.format(i)
            )
    model.optimize()
    cen_obj = model.ObjVal
    cen_result, matched_rider, macthed_driver = [], [], []
    pi_r, pi_d = {}, {}
    ...
    for (s, j, i1, i2, i3) in xindex.keys():
        if x[s, j, i1, i2, i3].x > 0:
            cen_result.append((s, j, i1, i2, i3))
            macthed_driver.append(j)
            pi_d[j] = pre_d[j]['mat'][s, j, i1, i2, i3]
            for i in ({i1, i2, i3} - {0}):
                matched_rider.append(i)
                pi_r[i] = pre_r[i]['mat'][s, j, i1, i2, i3]
    print('Dynamic centralized matching solution: obj {}, number of matches {}'.format(cen_obj, len(cen_result)))
    return round(cen_obj, 2), cen_result, matched_rider, macthed_driver, pi_r, pi_d